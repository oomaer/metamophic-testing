...
def run_fandango(grammar_path, log_file_path):
    ...
    log_print("=" * 80)
    log_print("Metamorphic Test: SMTP State Reset (RFC 5321)")
    log_print("A: Complete SMTP transaction on connection A")
    log_print("B: Same SMTP transaction after RSET on connection B")
    log_print("Relation: RSET must fully reset server state (same responses)")
    log_print("=" * 80)
    ...
    if violations:
        ...
        log_print("\nAll solutions passed - SMTP RSET works correctly!")
